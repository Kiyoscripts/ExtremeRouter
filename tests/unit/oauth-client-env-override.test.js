...
describe("OAuth client env override (Scenario A)", () => {
  ...
  it("falls back to packaged defaults when env is not set", async () => {
    ...
    vi.resetModules();
    const shared = await import("../../open-sse/providers/shared.js");
    expect(shared.GOOGLE_OAUTH_CLIENT.clientId).toBe("681255809395-oo8ft2oprdrnp9e3aqf6av3hmdib135j.apps.googleusercontent.com");
    expect(shared.GOOGLE_OAUTH_CLIENT.clientSecret).toBe("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");
    expect(shared.ANTIGRAVITY_OAUTH_CLIENT.clientSecret).toBe("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");
